    ...
    while (player.is_alive() && enemy.health > 0) {
        ...
        if (action == 1) {
            ...
            if (enemy.name.find("Малакар") != std::string::npos) {
                if (player.damage > 60) {
                    ...
                    std::cout << "[!] Магический щит Малакара поглотил избыточную энергию вашего читерского удара!\n";
                }
            ...
            if (chosen_potion == "Настойка_трав") {
                player.health += 15;
                std::cout << "\n[+] В спешке боя вы выпиваете Настойку трав и восстанавливаете 15 HP!\n";
            } else {
                player.health += 5;
                std::cout << "\n[+] Вы жадно осушаете кружку алкоголя прямо перед врагом! Восстановлено 5 HP!\n";
            }
        ...
        if (turn_passed) {
            ...
            if (enemy.name.find("Главарь") != std::string::npos || enemy.name.find("Малакар") != std::string::npos) {
                min_damage = enemy.damage * 30 / 100; 
            }
...
inline void interact_with_npcs() {
    ...
    if (chosen_npc == "Гид Олден") {
        ...
        std::cout << "3. Почему Цветочная долина не заражена металлом?\n";
        ...
        if (q == 1) {
            ...
            std::cout << "\n[Олден]: Из Дома ты можешь дойти до бескрайней Пустыни или прекрасной Цветочной поляны. \n[Олден]: Из поляны путь лежит в Дубовый лес, за которым скрывается Тёмный лес... \n[Олден]: Будь осторожен, дороги опасны!\n";
        } else if (q == 3) {
            std::cout << "\n[Олден]: Легенды гласят, что Цветочная поляна благословлена Древней Богиней Флорой. Земля там пропитана живой магией корней. \n[Олден]: Вирус пытается пробиться через Дубовый лес, но соки земли сжигают ржавчину. \n[Олден]: Однако, если эпицентр в замке разрастётся, магия природы падёт...\n";
        }
        wait_for_player();
    }   
        ...
        std::cout << "1. Дешевое пиво (5 золотых) -> Обычное пойло\n";
        std::cout << "2. Медовуха из Цветочной долины (12 золотых) -> Приятный сладкий вкус\n";
        std::cout << "3. Крепкий Эль гномов (25 золотых) -> Обжигает горло!\n"; // Исправлено: возвращен cout
        std::cout << "4. Королевское вино (50 золотых) -> Напиток для знати\n";
        std::cout << "5. Расскажи городские сплетни.\n6. Уйти\n\nВыбор: ";
        int q; std::cin >> q; std::cin.ignore(1, '\n');
        
        int cost = 0; std::string item_name = "";
        if (q == 1) { cost = 5; item_name = "Пиво"; }
        else if (q == 2) { cost = 12; item_name = "Медовуха"; }
        else if (q == 3) { cost = 25; item_name = "Крепкий_Эль"; }
        else if (q == 4) { cost = 50; item_name = "Вино"; }
        else if (q == 5) {
            clear_screen();
        ...
        std::cout << "--- ОРУЖИЕ ---\n";
        std::cout << "1. Охотничий нож (Урон +10 вместо меча) (Цена: 25 золотых)\n";
        std::cout << "2. Стальной длинный меч (Урон +25) (Цена: 60 золотых)\n";
        std::cout << "3. Тяжелая секира (Урон +45) (Цена: 110 золотых)\n\n";
        std::cout << "--- БРОНЯ ---\n";
        std::cout << "4. Кожаный доспех (Защита +2) (Цена: 30 золотых)\n";
        std::cout << "5. Стальная кираса (Защита +5) (Цена: 60 золотых)\n\n";
        std::cout << "--- ДРУГОЕ ---\n";
        std::cout << "6. Продать что-нибудь кузнецу\n";
        if (!player.has_flag("got_pickaxe")) {
            std::cout << "7. [Диалог] А что это за ржавая кирка в углу комнаты?\n";
        }
        std::cout << "0. Уйти\n\nВыбор: ";
        int q; std::cin >> q; std::cin.ignore(1, '\n');
        ...
        else if (q == 4 && player.gold >= 30) { player.gold -= 30; player.armor = 2; player.inventory.push_back("Кожаный_доспех"); std::cout << "Вы надели Кожаный доспех!\n"; }
        else if (q == 5 && player.gold >= 60) { player.gold -= 60; player.armor = 5; player.inventory.push_back("Стальная_кираса"); std::cout << "Вы надели Стальную кирасу!\n"; }
        else if (q == 6) {
        ...
        else if (q == 7 && !player.has_flag("got_pickaxe")) {
        ...
        int q; std::cin >> q; std::cin.ignore(1, '\n');
        if (q == 1) {
            Monster wolf = {"Зараженный Лютоволк", 80, 18, 45}; // ХП, Урон, Награда
            start_battle(wolf);
        }
    }
        ...
        if (player.has_flag("world_saved")) {
            ...
            std::cout << "1. [Работа] Разгрузить свежую рыбу (Займет 4 секунды)\n";
            std::cout << "2. Назад\nВыбор: ";
            int q; std::cin >> q; std::cin.ignore(1, '\n');
            clear_screen();
            ...
            if (q == 1) {
                ...
                clear_screen();
                player.gold += 20;
                std::cout << "\n[Бригадир]: Отличная работа, держи свои 20 золотых монеток!\n";
            }
            wait_for_player();
            return;
        }
        ...
        std::cout << "1. [Работа] Таскать тяжелые бочки (Займет 4 секунды)\n";
        ...
        if (q == 1) {
            ...
            clear_screen();
            player.gold += 20;
            std::cout << "\n[Бригадир]: Хорошо поработал, держи заслуженные 20 золотых монет!\n";
        } else if (q == 2) {
      ...
      std::cout << "[Раду]: Что тебе нужно?\n1. Вылечиться полностью (Цена: 20 золотых)\n2. Купить Настойку из трав (+15 HP) (Цена: 10 золотых)\n3. Уйти\nВыбор: ";
      int q; std::cin >> q; 
      std::cin.ignore(1, '\n');
      clear_screen();
      if (q == 1) {
      ...
      std::cout << "\n[Бернард]: Да, руки нет... Пришлось отрубить топором, пока живой металл не пожрал плечо. \n[Бернард]: На острове творился кошмар, люди кричали и превращались в статуи. \n[Бернард]: Не ходи туда.\n";
      wait_for_player();
    }
        ...
        if (player.has_flag("world_saved")) {
            if (player.has_flag("mira_alone")) {
                std::cout << "\n[Система] Лаборатория Миры пуста. Она так и не вернулась из своего похода в Проклятый замок...\n";
            } 
            else {
        ...
        if (player.has_flag("game_completed") && !player.has_flag("world_saved")) {
            ...
            if (has_ashes) {
                ...
                if (q == 1) {
                    ...
                    player.gold += 1000; 
                    world_map[locationID::cursed_castle].name = "Замок Малакра";
                    world_map[locationID::cursed_castle].description = "Разрушенный замок, в котором раньше обитал Малакр...";

                    std::cout << "[Мира]: Прах Малакара... В нём до сих пор теплится изначальная алхимия!\n";                    
                    ...
                    std::cout << "[Мира]: Путь туда смертельно опасен, но я пойду. Ты пойдёшь со мной, " << player.name << "?\n\n";
                    ...
                    if (choice_end == 1) {
                        ...
                        std::cout << "Мира устало улыбается вам и опирается на ваше плечо:\n";
                        std::cout << "[Мира]: Мы сделали это. Рука об руку... Спасибо тебе, мой герой.\n";
                        std::cout << "====================================================================\n";
                    } else {
                        ...
                        std::cout << "[+] Вы получили награду: 1000 золотых!\n";
                    }
                    wait_for_player();
                    return;
                }
        ...
        if (player.has_flag("mira_quest_started") && !player.has_flag("mira_quest_finished")) {
            ...
            if (has_metal) {
                ...
                if (q == 1) {
                    ...
                    player.gold += 150;
                    std::cout << "\n[Мира]: Спасибо тебе! Вот твоя награда — 150 золотых.\n[Мира]: Если Малакар разум уцелел, он до сих пор заперт в Проклятом замке. Если пойдешь туда — готовься к худшему.\n";
                }
            } else {
      ...
      if (player.has_flag("jafar_quest_finished")) {
        ...
        if (q == 1) {
            ...
            player.health -= 15;
            std::cout << "Вы теряете 15 HP, а на кристалле не осталось ни царапины. Нужен подходящий инструмент.\n";
        } 
        else if (q == 2) {
            ...
            if (has_pickaxe) {
                ...
                std::cout << "[+] В ваш инвентарь добавлен Образец_металла.\n";
            } else {
        ...
        if (q == 1) {
            ...
            // ЭФФЕКТ ПОЛНОГО ИСЦЕЛЕНИЯ ПЕРЕД БОЕМ
            player.health = player.max_health;
            ...
            Monster boss = {"Малакар, Повелитель Стали", 250, 35, 500};
            if (start_battle(boss)) {
                ...
                std::cout << " [Малакар]: Спасибо... путник... Ты освободил меня...\n";
